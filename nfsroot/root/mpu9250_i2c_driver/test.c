...
int main(){
   int ret = 0;
   float GyroX_old,GyroX_new,GyroY_old,GyroY_new,GyroZ_old,GyroZ_new;
   float AccelX_old,AccelX_new,AccelY_old,AccelY_new,AccelZ_old,AccelZ_new;
   float Temperature_old,Temperature_new;
   
   // Inicializamos las variables
   GyroX_old=0;GyroX_new=0;GyroY_old=0;GyroY_new=0;GyroZ_old=0;GyroZ_new=0;
   AccelX_old=0;AccelX_new=0;AccelY_old=0;AccelY_new=0;AccelZ_old=0;AccelZ_new=0;
   Temperature_old=0;Temperature_new=0;
   ...
   if (fd < 0){
	...
	while(1){
		...
		GyroY_old=GyroY_new;
		GyroZ_old=GyroZ_new;
		GyroX_new=mpu9250GetGyroX_rads(); 
		GyroY_new=mpu9250GetGyroY_rads();
		GyroZ_new=mpu9250GetGyroZ_rads();
	  
		AccelX_old=AccelX_new;
		AccelY_old=AccelY_new;
		AccelZ_old=AccelZ_new;
		AccelX_new=mpu9250GetAccelX_mss();
		AccelY_new=mpu9250GetAccelY_mss();
		AccelZ_new=mpu9250GetAccelZ_mss();
	  
		Temperature_old=Temperature_new;
		Temperature_new=mpu9250GetTemperature_C();
		
		system("clear");
		printf("===========================================================\r\n");
		printf("Giróscopo old:      (%3.4f, %3.4f, %3.4f)   [rad/s]\r\n",
				  GyroX_old,GyroY_old,GyroZ_old);
		printf("Giróscopo new:      (%3.4f, %3.4f, %3.4f)   [rad/s]\r\n",
				  GyroX_new,GyroY_new,GyroZ_new);
		printf("Delta Giróscopo:     (%3.4f, %3.4f, %3.4f)   [rad/s]\r\n\r\n",
				GyroX_new-GyroX_old,
				GyroY_new-GyroY_old,
				GyroZ_new-GyroZ_old
					);
		printf("Acelerómetro old:   (%3.4f, %3.4f, %3.4f)   [m/s2]\r\n",
				AccelX_old,AccelY_old,AccelZ_old);
		printf("Acelerómetro new:   (%3.4f, %3.4f, %3.4f)   [m/s2]\r\n",
				AccelX_new,AccelY_new,AccelZ_new);
		printf("Delta Acelerómetro: (%3.4f, %3.4f, %3.4f)   [m/s2]\r\n\r\n",
				AccelX_new-AccelX_old,
				AccelY_new-AccelY_old,
				AccelZ_new-AccelZ_old		
				);
		printf("Temperatura old:       %2.6f   [C]\r\n",
				  Temperature_old);
		printf("Temperatura new:       %2.6f   [C]\r\n",
				  Temperature_new);
  	    printf("Delta Temperatura :    %2.6f   [C]\r\n\r\n",
